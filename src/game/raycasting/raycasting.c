...
/*   Updated: 2022/08/21 11:21:34 by plavergn         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	ray->screenx = 2 * i / (float)WIN_WIDTH - 1;
	...
	ray->delta = create_vector(fabs(1 / ray->ray_dist.vx), \
		fabs(1 / ray->ray_dist.vy), 0);
...
void	do_raycast(t_game *game)
{
	...
	while (++index.vx < WIN_WIDTH)
	{
		...
		game->ray->size_cube = WIN_HEIGHT - HUD;
